...
- **Morph Pedal v3**: Complete UX redesign of embedded morph pedal firmware
  ...
  - Soft takeover — knobs lock after morph/preset/scroll until physical position matches current param value (5% threshold)
- **sonido-daisy: Edge snap** — knob values <0.5% snap to 0.0, >99.5% snap to 1.0 (avoids dead zones at pot extremes)
...
- **sonido-daisy: Modular control/audio layer** — 4 new library modules extracted from example code:
  - `controls.rs` — lock-free `ControlBuffer` with change detection, edge snap, LED bridge (pure `core`, no Embassy/alloc)
  - `effect_slot.rs` — `EffectSlot` (click-free bypass ramp, NaN/Inf sanitization, knob mapping), `BypassCrossfade`, `sanitize_stereo`, `CONTROL_POLL_EVERY`
